...
# Model Hyperparams
LEARNING_RATE =  float(os.environ.get("LEARNING_RATE", "0.001"))#学习率
EMBEDDING_DIM = int(os.environ.get("EMBEDDING_DIM", "50"))#
HIDDEN_DIM = int(os.environ.get("HIDDEN_DIM", "128"))#隐藏层
NUM_CLASSES = int(os.environ.get("NUM_CLASSES", "4"))#类别个数
WIND_SIZE = int(os.environ.get("WIND_SIZE", 7))

# Training parameters
NEPOCH = int(os.environ.get("NEPOCH", "20"))#训练次数
...
def sgd_callback(model, num_example_seen):
    loss = model.calculate_loss(X_test, y_test)#计算一次损失
